...
def get_traces_data(inkml_file_abs_path, xmlns="{http://www.w3.org/2003/InkML}"):
    ...
    equation = None
    for annotation in root.iter(doc_namespace + "annotation"):
        if annotation.get("type") == "truth":
            equation = annotation.text
            break
...
def get_output_path(input_path, output_dir):
    output_file = input_path.split("/")[-1].split(".inkml")[0]
    return output_dir + "/" + output_file + ".png"


def inkml2img(input_path, output_path, color="black"):
    traces, equation = get_traces_data(input_path)
...
def convert_dir(input_dir, output_dir):
    df = pd.DataFrame(columns=["formula", "image"])
    file_extension = "*.inkml"
    files = glob.glob(input_dir + "/" + file_extension)
    for file in files:
        output_path = get_output_path(file, output_dir)
        equation = inkml2img(file, output_path)
        new_entry = pd.Series({"image": output_path, "formula": equation})
...
if __name__ == "__main__":
    ...
    convert_dir(input_dir, output_path)
